...
export default function Step1Page() {
  ...
  const [authStep, setAuthStep] = useState<'auth' | 'verify-email'>('auth');
  ...
  const handleEmailVerified = () => {
    // Código verificado com sucesso, salvar dados e ir para step2
    const onboardingData = JSON.parse(sessionStorage.getItem('onboardingData') || '{}');
    const updatedData = {
      ...onboardingData,
      email: formData.email,
  ...
  const handleGoogleAuth = () => {
    ...
    setTimeout(() => {
      // Salvar dados e ir direto para step2
      const onboardingData = JSON.parse(sessionStorage.getItem('onboardingData') || '{}');
      const updatedData = {
        ...onboardingData,
        email: 'usuario@gmail.com', // Email vindo do Google
        isEmailVerified: true,
        authMethod: 'google'
      };
      sessionStorage.setItem('onboardingData', JSON.stringify(updatedData));
  ...
  return (
    ...
    >
      <div className="bg-white rounded-xl shadow-lg p-8">
        ...
        {/* Botão Google Auth - sempre visível */}
        <div className="mb-6">
          <button
        ...
        {/* Email Authentication Form - só mostra quando NÃO está verificando */}
        {authStep === 'auth' && (
