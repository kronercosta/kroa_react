    ...
    permissoes: {
      agenda: {
        ...
        acoes: {
          ...
          evoluir: { ativo: false, profissionais: [] }
        }
      },
      pacientes: {
        ativo: true,
    ...
    setFormData(prev => ({
      ...
      permissoes: {
        ...
        [module]: {
          ...prev.permissoes[module as keyof typeof prev.permissoes],
          acoes: {
            ...prev.permissoes[module as keyof typeof prev.permissoes].acoes,
            [action]: {
              ...prev.permissoes[module as keyof typeof prev.permissoes].acoes[action],
              ativo
            }
          }
        }
            ...
            <tbody className="bg-white">
              {actions.map((action, index) => (
                <tr key={action.key} className={index < actions.length - 1 ? 'border-b border-gray-100' : ''}>
                  ...
                  <td className="py-3 px-4 text-center">
                    <Switch
                      checked={module.acoes[action.key]?.ativo || false}
